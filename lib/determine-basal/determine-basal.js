...
var determine_basal = function determine_basal(glucose_status, currenttemp, iob_data, profile, autosens_data, meal_data, tempBasalFunctions, microBolusAllowed, reservoir_data) {
    ...
    if (iob_data.iob > max_iob) {
        ...
        if (microBolusAllowed && enableSMB && bg > threshold) {
            ...
            console.error("90m COBpredBG:",round(COBpredBGs[Math.min(insulinPeak5m,COBpredBGs.length-1)]),"UAMpredBGs:",round(UAMpredBGs[Math.min(insulinPeak5m,UAMpredBGs.length-1)]),"IOBpredBGs:",round(IOBpredBGs[Math.min(insulinPeak5m,IOBpredBGs.length-1)]), "Average:",predBG90m);
            ...
            // if insulinReq is positive and no boluses have been given with 15m,
            // set a high temp to cover insulinReq, and try to SMB.
            // once an SMB is successful, we can ZT again.
            if ( insulinReq > 0 && lastBolusAge > 15 && noZTInsulinReq > 0 ) {
                durationReq = 0;
            } else {
