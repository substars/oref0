...
var determine_basal = function determine_basal(glucose_status, currenttemp, iob_data, profile, autosens_data, meal_data, tempBasalFunctions, microBolusAllowed, reservoir_data) {
    ...
    // if not in LGS mode, and meal_data.currentDeviation is missing, do nothing
    if ( typeof(meal_data.currentDeviation) !== 'number' ) {
        rT.reason += JSON.stringify(meal_data) + "; doing nothing. ";
        return rT;
    }

    if (enableSMB && microBolusAllowed) {
        // if minZTGuardBG is above target and deviations are positive, then
        // make sure insulinReq is high enough to bring minZTGuardBG down to target
        var ZTInsulinReq = (minZTGuardBG - target_bg) / sens;
        ZTInsulinReq = round(ZTInsulinReq,2);
    } else {
        ZTInsulinReq = 0;
    }
    console.error("ZTInsulinReq:",ZTInsulinReq,"minZTGuardBG:",minZTGuardBG,"target_bg:",target_bg);
    ...
    if (eventualBG < min_bg && ! ZTInsulinReq > 0) { // if eventual BG is below target:
    ...
    if (Math.min(eventualBG,minPredBG) < max_bg && ! ZTInsulinReq > 0) {
        // if in SMB mode, don't cancel SMB zero temp
        if (! (microBolusAllowed && enableSMB )) {
    ...
    if (iob_data.iob > max_iob) {
        ...
        // insulinReq is the additional insulin required to get minPredBG or eventualBG down to target_bg
        //console.error(minPredBG,eventualBG);
        var insulinReq = (Math.min(minPredBG,eventualBG) - target_bg) / sens;
        insulinReq = round(insulinReq,2);
        if (microBolusAllowed && enableSMB && minZTGuardBG > target_bg && deviation > 0 && ZTInsulinReq > insulinReq) {
            console.error("Adjusting insulinReq from",insulinReq,"to",ZTInsulinReq,"for minZTGuardBG",minZTGuardBG);
            insulinReq = ZTInsulinReq;
        }
        ...
        if (microBolusAllowed && enableSMB && bg > threshold) {
            ...
            // calculate the predBG 90m from now
            var predBGs90m = [ COBpredBGs[insulinPeak5m], UAMpredBGs[insulinPeak5m], IOBpredBGs[insulinPeak5m] ];
            var sum = IOBpredBGs[Math.min(insulinPeak5m,IOBpredBGs.length-1)];
            var count = 1;
            if (COBpredBGs[Math.min(insulinPeak5m,COBpredBGs.length-1)]) {
                sum += COBpredBGs[Math.min(insulinPeak5m,COBpredBGs.length-1)];
                count++;
            }
            //else if ( COBpredBGs ) { sum += COBpredBGs[COBpredBGs.length-1]; count++; }
            if (UAMpredBGs[Math.min(insulinPeak5m,UAMpredBGs.length-1)]) {
                sum += UAMpredBGs[Math.min(insulinPeak5m,UAMpredBGs.length-1)];
                count++;
            }
            //else if ( UAMpredBGs ) { sum += UAMpredBGs[UAMpredBGs.length-1]; count++; }
            predBG90m = Math.round( sum / count );
            console.error("90m COBpredBG:",round(COBpredBGs[Math.min(insulinPeak5m,COBpredBGs.length-1)]),"UAMpredBG:",round(UAMpredBGs[Math.min(insulinPeak5m,UAMpredBGs.length-1)]),"IOBpredBG:",round(IOBpredBGs[Math.min(insulinPeak5m,IOBpredBGs.length-1)]), "Average:",predBG90m);

            // if that predBG is below target, leave insulinReq unchanged.
            // if that predBG is > target, then add 30m of basal to insulinReq.
            var noZTInsulinReq = insulinReq;
            if ( profile.temptargetSet && target_bg > normalTarget ) {
                console.error("Temp target of",target_bg,"set: not adjusting insulinReq of",insulinReq);
            } else if ( minZTGuardBG <= threshold ) {
                console.error("minZTGuardBG",minZTGuardBG,"<",threshold,"- not adjusting insulinReq of",insulinReq);
            } else if ( predBG90m > target_bg ) {
                var adjustment = round(Math.min( basal/2, (predBG90m-target_bg)/sens ),2);
                insulinReq += adjustment;
                insulinReq = round(insulinReq,3);
                rT.insulinReq = insulinReq;
                console.error("Increasing insulinReq by",adjustment,"from",noZTInsulinReq,"to",insulinReq);
            }
            ...
            // if insulinReq is positive and no boluses have been given with 30m,
            // set a high temp to cover insulinReq, and try to SMB.
            // once an SMB is successful, we can ZT again.
            if ( insulinReq > 0 && lastBolusAge > 30 && noZTInsulinReq > 0 ) {
                durationReq = 0;
            } else {
                durationReq = Math.max(durationReq, 30);
            }
